...
import { X, Plus, Trash2, AlertCircle, CheckCircle, Loader, Upload, Lock, Pencil, Search, BarChart2, Download, RotateCcw, TableProperties } from 'lucide-react';
...
const ReservationSearchTab: React.FC<ReservationSearchTabProps> = ({ records }) => {
        ...
        <div className="flex items-start justify-center min-h-screen p-2 sm:p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl my-4 sm:my-8">
            ...
            <div className="flex border-b border-gray-200 px-4 sm:px-6">
              {([
                { key: 'search', label: '예약 현황 조회', icon: <TableProperties className="w-4 h-4" /> },
                { key: 'manage', label: '관리',           icon: <Upload className="w-4 h-4" /> },
              ] as const).map(tab => (
            ...
            <div className="p-4 sm:p-6 space-y-6">
              ...
              {activeTab === 'search' ? (
                /* ── 예약 현황 조회 탭 ── */
                <ReservationSearchTab records={records.filter(r => !CANCELLED.includes(r.status))} />
              ) : (
              <>
              {/* ── 가동률 ── */}
